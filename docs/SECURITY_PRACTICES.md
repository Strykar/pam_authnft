...
| **Kernel-matrix audit (tier 2)** | `.github/workflows/audit-vm.yml` → `make audit-vm` / `make audit-vm-matrix` (virtme-ng, `/dev/kvm`) | nightly + dispatch | the socket-cgroupv2 packet-match invariants (`tests/packet_match_headless.sh`: allowed/disallowed match, alloc-time invariant, per-session isolation) plus unit + seccomp enforcement + the fault matrix, under a real, pinnable kernel; the only tier that sweeps a kernel matrix, so `socket cgroupv2 level 2` is proven across kernel versions. The full integration suite (Part B) is excluded here (host coupling) |
